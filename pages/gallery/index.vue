<template>
    <div class="container mx-auto px-4 py-8">
        <div v-if="galleryPending" class="text-center py-12">
            ...
            <p class="mt-4 text-gray-700">Loading gallery...</p>
        </div>

        <div v-else-if="galleryError" class="text-center text-red-500 py-12">
            <p>Error loading gallery: {{ galleryError.message }}</p>
        ...
        <div v-else class="text-center text-gray-600 py-12">
            <p class="text-xl font-semibold">No images in the gallery.</p>
            <p class="mt-2">Check back later for new collections!</p>
        </div>
...
const handleModalImageError = () => {
    modalImageLoading.value = false;
    if (images.value && images.value[currentImage.value]) {
        console.error('Failed to load modal image:', images.value[currentImage.value]);
    } else {
        console.error('Failed to load modal image at unknown index.');
    }
};

const handleKeydown = (event) => {
    if (!showModal.value) return;
    if (event.key === 'ArrowRight' && (!images.value || images.value.length > 1)) {
        nextImage();
    } else if (event.key === 'ArrowLeft' && (!images.value || images.value.length > 1)) {
        prevImage();
    }
};
...
useHead({
    ...
    meta: [
        ...
        { property: 'og:title', content: 'Plant Gallery - Plant Shop' },
        { property: 'og:description', content: 'Explore our beautiful and diverse plant gallery.' },
        { property: 'og:image', content: '/social-share-image.jpg' },
        { property: 'og:url', content: 'http://localhost:8080/gallery' }
    ]
});
</script>
